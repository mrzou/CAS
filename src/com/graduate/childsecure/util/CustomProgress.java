...
public class CustomProgress extends Dialog {
	...
	public void setMessage(CharSequence message) {
		if (message != null && message.length() > 0) {
			findViewById(R.id.diolog_message).setVisibility(View.VISIBLE);
			TextView txt = (TextView) findViewById(R.id.diolog_message);
			txt.setText(message);
			txt.invalidate();
		}
	}
	...
	public static CustomProgress show(Context context, CharSequence message, boolean cancelable, OnCancelListener cancelListener) {
		...
		if (message == null || message.length() == 0) {
			dialog.findViewById(R.id.diolog_message).setVisibility(View.GONE);
		} else {
			TextView txt = (TextView) dialog.findViewById(R.id.diolog_message);
			txt.setText(message);
		}
		// 按返回键是否取消
